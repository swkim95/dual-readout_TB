...
for mid in range(1,16):
    WaveDat=f"/Users/swkim/DRC/dual-readout_TB/Run_{rn}/Run_{rn}_Wave/Run_{rn}_Wave_MID_{mid}/Run_{rn}_Wave_MID_{mid}_FILE_{fn}.dat"
    FastDat=f"/Users/swkim/DRC/dual-readout_TB/Run_{rn}/Run_{rn}_Fast/Run_{rn}_Fast_MID_{mid}/Run_{rn}_Fast_MID_{mid}_FILE_{fn}.dat"

    if(os.path.isfile(WaveDat)):
        WaveFiles.append(WaveDat)
    else:
        print(WaveDat,"not found")

    if(os.path.isfile(FastDat)):
