...
# /pnfs/knu.ac.kr/data/cms/store/user/sungwon/2022_DRC_TB_Data

## Do wave
if not args.doFast :
    waveFileNumbers = []
    for mid in range(1,16):
        WaveDat=f"/pnfs/knu.ac.kr/data/cms/store/user/sungwon/2022_DRC_TB_Data/HDD_Run_{rn}_validated/Run_{rn}_Wave/Run_{rn}_Wave_MID_{mid}/"
    ...
    for fn in range(maxWaveFileNum) :
        WaveFilesPerFN = []
        for mid in range(1,16) :
            fileName=f"/pnfs/knu.ac.kr/data/cms/store/user/sungwon/2022_DRC_TB_Data/HDD_Run_{rn}_validated/Run_{rn}_Wave/Run_{rn}_Wave_MID_{mid}/Run_{rn}_Wave_MID_{mid}_FILE_{fn}.dat"
    ...
    nameWave = f"/u/user/swkim/SE_UserHome/2022_DRC_TB_ntuple/waveform/Run_{rn}/ntuple_Run_{rn}_Wave"
...
if args.doFast :
    fastFileNumbers = []
    for mid in range(1,16):
        FastDat=f"/pnfs/knu.ac.kr/data/cms/store/user/sungwon/2022_DRC_TB_Data/HDD_Run_{rn}_validated/Run_{rn}_Fast/Run_{rn}_Fast_MID_{mid}/"
    ...
    for fn in range(maxFastFileNum) :
        FastFilesPerFN = []
        for mid in range(1,16) :
            fileName=f"/pnfs/knu.ac.kr/data/cms/store/user/sungwon/2022_DRC_TB_Data/HDD_Run_{rn}_validated/Run_{rn}_Fast/Run_{rn}_Fast_MID_{mid}/Run_{rn}_Fast_MID_{mid}_FILE_{fn}.dat"
            if(os.path.isfile(fileName)) :
    ...
    nameFast = f"/u/user/swkim/SE_UserHome/2022_DRC_TB_ntuple/fastmode/Run_{rn}/ntuple_Run_{rn}_Fast"
